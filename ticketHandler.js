...
module.exports = (client) => {
  ...
  client.on('messageCreate', async (message) => {
    ...
    // Odczytaj numer z nazwy kanału, np. "legit-rep-7314" → 7314
    ...
    const legitNumber = currentNumber !== null ? currentNumber : '?';

    // Usuń wiadomość właściciela/realizatora
    await message.delete().catch(() => {});

    // Wyślij embed z zdjęciem
    const embed = new EmbedBuilder()
      .setColor(0x57f287)
      .setTitle(`✅ LEGIT CHECK → ${legitNumber}`)
      .addFields(
        { name: ':wymiensianoksa: Kupno itemów', value: `>>> **${config.legitServerName}**`, inline: false },
        { name: ':wymiensianoksa: Klient potwierdził udaną transakcję', value: '\u200b', inline: false },
        { name: ':anashop: ANA SHOP — Bezpieczne zakupy', value: '\u200b', inline: false },
      )
      .setImage(image.url)
      .setFooter({ text: 'ANA SHOP • Najlepszy sklep' })
      .setTimestamp();
