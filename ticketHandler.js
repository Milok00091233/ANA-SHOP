...
module.exports = (client) => {
  ...
  client.on('messageCreate', async (message) => {
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          },
          {
            name: '\u200b',
            value:
              'Na czym polega **system legit-checków?** klient po zakupie ma **5 minut na decyzję** czy chce **dobrowolnie potwierdzić** ' +
              'transakcję, jeżeli nie wybierze żadnych z opcji (wystawia/nie wystawia) **bot robi to automatycznie** wysyłając wiadomość ' +
              `z jego profilowym oraz nazwą na kanał <#${config.legitChannelId}>.`,
          }
        )
        .setFooter({ text: 'ANA SHOP • Najlepszy sklep' });
    ...
    // +rep → legit check embed
    if (message.content.startsWith('+rep') && message.channel.id === config.legitChannelId) {
      const tresc = message.content.slice(4).trim(); // wszystko po "+rep"
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: '» PRZYKŁAD',
            value: '```' + `+rep ${tresc || '@realizator kupno NAZWA_SERWERA 100.00 PLN'}` + '```',
          },
          {
            name: '​',
            value:
              'Na czym polega **system legit-checków?** klient po zakupie ma **5 minut na decyzję** czy chce **dobrowolnie potwierdzić** ' +
              'transakcję, jeżeli nie wybierze żadnych z opcji (wystawia/nie wystawia) **bot robi to automatycznie** wysyłając **wiadomość** z ' +
              `jego profilowym oraz nazwą na kanał <#${message.channel.id}>`,
          }
        )
        .setFooter({ text: 'ANA SHOP • Najlepszy sklep' })
  ...
  // ─────────────────────────────────────────────
  // LEGIT CHECK — kanał z +rep
  // ─────────────────────────────────────────────
  client.on('messageCreate', async (message) => {
    if (message.author.bot) return;
    if (!message.guild) return;

    // Sprawdź czy to kanał legit-check (nazwa zawiera "legit" lub "rep")
    const isLegitChannel =
      message.channel.name.includes('legit') ||
      message.channel.name.includes('rep');
    if (!isLegitChannel) return;

    // Sprawdź czy wiadomość zaczyna się od +rep
    if (!message.content.toLowerCase().startsWith('+rep')) return;
