...
module.exports = (client) => {
  ...
  client.on('interactionCreate', async (interaction) => {
    ...
    // ── Kalkulator: "Ile otrzymam?" → modal ──
    if (interaction.isButton() && interaction.customId === 'calc_ile_otrzymam') {
      const modal = new ModalBuilder()
        .setCustomId('calc_modal_otrzymam')
        .setTitle('Obliczanie...');
      ...
      modal.addComponents(
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
            ...
            .setPlaceholder('Przykład: 150 (w PLN)')
            .setRequired(true)
        ),
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
            .setCustomId('calc_metoda')
            .setLabel('Metoda Płatności (wpisz numer):')
            .setStyle(TextInputStyle.Short)
            .setPlaceholder('1=PSC+paragon 2=PSC brak 3=BLIK przelew 4=Kod BLIK 5=PayPal 6=Krypto')
            .setRequired(true)
        )
      );
    ...
    // ── Kalkulator: "Ile muszę dać?" → modal ──
    if (interaction.isButton() && interaction.customId === 'calc_ile_musze_dac') {
      const modal = new ModalBuilder()
        .setCustomId('calc_modal_musze_dac')
        .setTitle('Obliczanie...');
      ...
      modal.addComponents(
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
            .setCustomId('calc_kwota')
            .setLabel('Kwota waluty (np. 125K lub 125000):')
            .setStyle(TextInputStyle.Short)
            .setPlaceholder('Przykład: 125K lub 125000')
            .setRequired(true)
        ),
        new ActionRowBuilder().addComponents(
          new TextInputBuilder()
    ...
    // ── Kalkulator: wyniki modali ──
    if (interaction.isModalSubmit() && (interaction.customId === 'calc_modal_otrzymam' || interaction.customId === 'calc_modal_musze_dac')) {
      const KURS = 6500; // 1 PLN = 6500 waluty
      const PROWIZJE = {
        '1': { nazwa: '💳 PSC z paragonem',        procent: 15  },
        '2': { nazwa: '💳 PSC bez paragonu',        procent: 25  },
        '3': { nazwa: '📱 BLIK (przelew)',           procent: 0   },
        '4': { nazwa: '📱 Kod BLIK',                procent: 10  },
        '5': { nazwa: '🅿️ PayPal',                  procent: 10  },
        '6': { nazwa: '🪙 Kryptowaluty (LTC/SOL/BTC)', procent: 0 },
      };

      const rawKwota  = interaction.fields.getTextInputValue('calc_kwota').trim().replace(/,/g, '.').replace(/[kK]$/, (m) => m ? '000' : '');
      const rawMetoda = interaction.fields.getTextInputValue('calc_metoda').trim();
      const metoda    = PROWIZJE[rawMetoda];

      if (!metoda) {
        await interaction.reply({ content: '❌ Nieprawidłowy numer metody płatności! Wpisz liczbę 1–6.', ephemeral: true });
        return;
      }
      ...
      if (interaction.customId === 'calc_modal_otrzymam') {
        // PLN → waluta
        const netPLN     = kwota * (1 - prowizjaUlamek);
        const waluta     = Math.floor(netPLN * KURS);
        const prowizjaPLN = kwota * prowizjaUlamek;
        wynikText =
          `💵 Wpłacasz: **${kwota.toFixed(2)} PLN**\n` +
          `💸 Prowizja (${metoda.procent}%): **${prowizjaPLN.toFixed(2)} PLN**\n` +
          `🎮 Otrzymasz: **${waluta.toLocaleString('pl-PL')} waluty**`;
      } else {
        // waluta → PLN
        const brutoPLN   = (kwota / KURS) / (1 - prowizjaUlamek);
        const prowizjaPLN = brutoPLN * prowizjaUlamek;
        wynikText =
